---
title: "Deploying Eleventy to NeoCities with CI/CD"
date: 2026-01-01
description: "A guide to automatically deploying your Eleventy static site to NeoCities using CI/CD, including authentication, error handling, and optimizations."
postType: article
tags: ["deployment", "cicd", "neocities", "tutorial"]
featured_image: /assets/images/posts/deploying-to-neocities.jpg
...
NeoCities is a free web hosting service embracing the spirit of the early web, taking its name from the tragically-defunct GeoCities. It offers a generous free tier: 1 GB of storage and 200 GB of bandwidth, with no server management or complex configuration.

This guide shows you how to automatically deploy your Eleventy static site to NeoCities using CI/CD, uploading the rendered `_site` output via their API.

## Why NeoCities?

- **Free hosting**: 1 GB storage, 200 GB bandwidth
- **No server management**: Just upload files
- **Early web spirit**: Community-focused, retro aesthetic
- **Backup option**: Mirror your site as a backup following the 3-2-1 rule
- **Community**: Join the NeoCities community

## The Plan

1. Automatically deploy your Eleventy site to NeoCities with each commit
...
First, add your credentials to your CI/CD platform's environment variables. If using GitLab, go to Settings → CI/CD → Variables:
...
- **Automatic**: Every commit to the main branch (since that's usually when you've actually fixed something)
- **Manual**: You can trigger it from your CI/CD platform's web interface - handy for those "oops, I need to push that again" moments
...
If using GitLab, there are a couple of nice-to-have features you can enable. These don't affect the deployment itself:

- **Artifacts**: The `_site` folder gets saved, so you can download the built site if needed
- **Environment**: Tracks this as a "production" environment with your NeoCities URL
...
The script is written so that authentication is tested _before_ uploading atttempts. Your CI/CD platform provides detailed logs for deployment. You can visit your NeoCities site to confirm uploads, and make small changes to ensure only modified files are uploaded.
...
If you do use GitLab, it seems important to make sure your variables are stored in GitLab settings instead of being present anywhere in your repository (this would give access of your password/API key to anybody). Also ensure neither the "protect variable" nor the "expand variable reference" flags are checked. 
...
- [GitLab CI/CD Documentation](https://docs.gitlab.com/ee/ci/) (if using GitLab)
- [GitHub Actions Documentation](https://docs.github.com/en/actions) (if using GitHub)
