---
title: "Cutting Netlify Build Time in Half"
date: 2026-07-15
description: "How to optimize Netlify build times by caching data fetches, removing unused webhooks, and reducing build minutes."
postType: article
tags: ["eleventy", "netlify", "performance", "deployment", "tutorial"]
featured_image: /assets/images/posts/netlify-build-optimization.jpg
...
After optimizing your Eleventy build locally, you might still face build time limits on your hosting platform. Netlify's free tier gives you 300 build minutes per month. If your build takes 2-3 minutes and you deploy daily, you'll hit that limit quickly.

This guide focuses on Netlify-specific optimizations to reduce build time and stay within free tier limits.
...
My `lastModified` filter (the one from the last post's git-log optimization) runs `git log --name-only` across the *entire* history to build its timestamp cache. On a blobless clone, that can trigger a lazy blob fetch for old file versions git doesn't have locally yet, so that fetch goes out over SSH to your Git host, which the Netlify build server has no reason to have credentials for (it authenticates through its own integration, not a general-purpose SSH key).
